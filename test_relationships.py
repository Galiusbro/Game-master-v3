#!/usr/bin/env python3
"""
Тестовый скрипт для демонстрации работы с relationship_to_player
"""
...
async def test_relationship_system():
    ...
    # 4. Обновляем relationship_to_player в NPC
    ...
    if response:
        ...
        # Обновляем relationship_to_player
        ...
        if response:
            ...
            relationship = updated_npc['current_state']['relationship_to_player'].get(player_id)
    ...
    # 6. Получаем контекст NPC для проверки отношений
    print("\n🔍 Получаем контекст NPC...")
    response, status = make_request(f"/entities/{npc_id}/context")
    if response:
        print(f"✅ Контекст получен: найдено {len(response)} связанных сущностей")
        for entity in response:
            print(f"   - {entity['entity']['name']} ({entity['entity_type']})")
    else:
        print(f"❌ Ошибка получения контекста: {status}")
    
    # 7. Демонстрируем различные типы отношений
