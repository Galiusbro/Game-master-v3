#!/usr/bin/env python3
"""
Исправленный тестовый скрипт для демонстрации работы с relationship_to_player
"""
...
async def test_relationship_system():
    ...
    print("🔗 ТЕСТ СИСТЕМЫ ОТНОШЕНИЙ (ИСПРАВЛЕННАЯ ВЕРСИЯ)")
    print("=" * 60)
    ...
    if response:
        ...
        # Отправляем обновление (используем только entity_data)
        update_data = {
            "entity_data": npc,
            "actor_id": str(uuid.uuid4()),
            "session_id": str(uuid.uuid4())
